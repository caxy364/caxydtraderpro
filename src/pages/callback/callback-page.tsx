...
const CallbackPage: React.FC = () => {
    ...
    useEffect(() => {
        const handleCallback = async () => {
            ...
            // ── Legacy login: Deriv redirects to /callback with acct1/token1/curr1 ──
            // This happens when no redirect_uri is passed to oauth.deriv.com and the
            // registered URI for the app points here. Must be handled before the PKCE
            // code check or the tokens get discarded and the user sees an error.
            const acct1 = searchParams.get('acct1');
            const token1 = searchParams.get('token1');

            if (acct1 && token1) {
                console.log('[Callback] Legacy login params detected for account:', acct1);
                try {
                    const accountsList: Record<string, string> = {};
                    const clientAccounts: Record<string, any> = {};
                    const accountListForStore: any[] = [];

                    // Collect every acct/token/curr pair Deriv sends (acct1…acctN)
                    let idx = 1;
